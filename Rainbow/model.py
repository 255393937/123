...
class NoisyLinear(nn.Module): # Noisy networks
	...
	"""
	def __init__(self, in_features, out_features, std_init=0.5): # std 를 0.5 정도로는 해야 exploration이 잘 되는 것을 확인함
		...
		self.register_buffer('weight_epsilon', torch.empty(out_features, in_features)) # register_buffer는 모델 parameter로 등록되지 않음 -> update는 안됨
		self.register_buffer('bias_epsilon', torch.empty(out_features)) # noise를 주는 것 VAE에서 noise랑 비슷한 것
	...
	def _scale_noise(self, size): 
		x = torch.randn(size, device=self.weight_mu.device) # normal distribution 으로 reset
		return x.sign().mul_(x.abs().sqrt_()) # root scale로 만들어줌

	def reset_noise(self):
		epsilon_in = self._scale_noise(self.in_features) # root normal distribution noise
		epsilon_out = self._scale_noise(self.out_features) 
		self.weight_epsilon.copy_(epsilon_out.outer(epsilon_in)) # outer product 해서 weight epsilon에 넣음 (dim 맞추려고)
		self.bias_epsilon.copy_(epsilon_out)

	def forward(self, input):
		if self.training: # .train()을 해놓은 network에 대해서는 noisy net 적용
...
class DQN(nn.Module): # RainbowDQN
	...
	def forward(self, x, log=False):
		...
		if log:  # log 확률을 구하는 것
			q = F.log_softmax(q, dim=2)
		else: # 그냥 확률 구하는 것
			q = F.softmax(q, dim=2)
		return q

	def reset_noise(self): # noise를 주기적으로 reset -> 안하면 noise 안사용하도록 학습해버리기 때문
