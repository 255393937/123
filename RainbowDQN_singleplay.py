...
def train():
    date = datetime.now()
    summary = SummaryWriter(os.path.join(args.tensorboard,"rainbowDQN", date.strftime("%Y%b%d_%H_%M_%S"))) # 입력받은 경로에 저장 Tensorboard
    ...
    priority_weight_increase = (1 - args.priority_weight) / (args.T_max - args.learn_start) # important sampling 에 쓰이는 것
    ...
    for T in trange(1, args.T_max + 1): # num of time step
        if done:
            ...
            if episode % 10 == 0 and episode != 0:
                ...
                # summary.add_scalars('Action probability', {
                #     'left' : action_count[0]/action_count.sum(),
                #     'idle' : action_count[1]/action_count.sum(),
                #     'right' : action_count[2]/action_count.sum(),
                #     'fast' : action_count[3]/action_count.sum(),
                #     'slow' : action_count[4]/action_count.sum(),
                # }, episode)
                r_episode = 0
        ...
        if args.reward_clip > 0:
            reward = max(min(reward, args.reward_clip), -args.reward_clip) # reward clipping -> distribution RL 성능 올리기 위함
        
        mem.append(state, action, reward, done) # replay experience에 저장

        if T >= args.learn_start: # replay buffer 초반에 쌓기
            # Anneal importance sampling weight β to 1 (importance factor 조절하는 것)
...
def test():
    ...
    if not os.path.exists("/".join(args.video_path.split('/')[:-1])):
        os.path.makedirs("/".join(args.video_path.split('/')[:-1]))
    vid = video_recorder.VideoRecorder(env, metadata={'video.frames_per_second' : 15,
                                                        'video.output_frames_per_second': 30}, path=args.video_path)
    env.set_rendering_callback(vid.capture_frame)
    state = env.reset()
